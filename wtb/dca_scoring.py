...
from .level_engine import validate_plan_levels
...
def compute_execution_plan(
    ...
    # Centralized validation guard (shared with main algo)
    entry_zone_lower, entry_zone_upper, stop_loss, [take_profit_1, take_profit_2, take_profit_3], was_repaired, repair_reason = validate_plan_levels(
        side=side,
        entry_low=entry_zone_lower,
        entry_high=entry_zone_upper,
        sl=stop_loss,
        tps=[take_profit_1, take_profit_2, take_profit_3],
        current_price=current_price,
    )
    if was_repaired:
        validation_flags.append(f"REPAIRED:{repair_reason}")

    # Recompute RR after potential repair
    entry_mid = (entry_zone_lower + entry_zone_upper) / 2.0
    risk = abs(entry_mid - stop_loss)
    if risk > 0:
        rr_tp1 = abs(take_profit_1 - entry_mid) / risk
