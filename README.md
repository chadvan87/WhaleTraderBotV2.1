...
# Enable Ollama psychology overlay
python whaletraderbot.py scan --side LONG --ollama

# Scan for SHORT setups
python whaletraderbot.py scan --side SHORT

chmod +x "/Users/van/Desktop/Crypto/futures_bot 2/run"
# Shortcuts
./run long
./run short

# Scalp mode (faster TFs + stricter confidence)
./run scalp --side LONG
...
```bash
python whaletraderbot.py scan --side LONG [--top 120] [--watchlist-k 10] [--min-score 60] [--ollama] [--no-derivs] [--no-orderflow]
```
...
| `--ollama`      | false   | Enable local Ollama overlay (Plutus)         |
| `--no-derivs`   | false   | Disable derivatives overlay                  |
| `--no-orderflow`| false   | Disable orderflow overlay                    |

### Scalp Mode

Scalp mode runs the same pipeline but applies config overrides (faster timeframes + stricter confidence thresholds) so you can trade smaller, quicker moves on “bad days”.

```bash
./run scalp --side LONG [--ollama]
```
...
| `--bars`        | 500     | Total historical bars to download                    |
...
1. Downloads historical klines from Binance
...
Every candidate gets a lightweight **confidence score** (0–100) and label:
- `EXECUTE`: high-confidence only
- `WATCH`: decent setup but needs discretion
- `SKIP`: fail hard gate

Tune thresholds in `confidence`:
...
### External intel (optional)

External intel is **disabled by default** and only runs for the top N candidates (to keep runs fast). It’s designed to plug in Grok/news/on-chain/event checks without changing deterministic ALGO entry/SL/TP.

Providers live in `wtb/external_intel_providers.py`:
- `mock` (default): neutral placeholders
- `http`: call your own bridge service that returns JSON
...
`./run scalp` applies override settings from `scalp.overrides` (faster TFs + tighter/stricter gates). Adjust that block to match how you scalp.
...
│   ├── confidence.py        # Heuristic confidence gating (EXECUTE/WATCH/SKIP)
│   ├── external_intel.py    # External intel cache + aggregation (optional)
│   ├── external_intel_providers.py # Provider interface (mock/http)
│   ├── ui.py                # Rich progress helpers
